...
TimeLimitedCache.prototype.set = function (key, value, duration) {
    ...
    expirationTime.setMilliseconds(expirationTime.getMilliseconds() + duration);
    const valObj = {
        value,
        expirationTime,
    };
    // Check key and timestamp
    if (
        this.cache.has(key) &&
        new Date() < this.cache.get(key).expirationTime
    ) {
...
TimeLimitedCache.prototype.get = function (key) {
    if (
        ...
        const valObj = this.cache.get(key);
        return valObj.value;
    }
    return -1;
};
...
TimeLimitedCache.prototype.count = function () {
    ...
    for (const key of this.cache.keys()) {
        const valObj = this.cache.get(key);
        if (new Date() < valObj.expirationTime) {
            totalCount += 1;
        }
    }
    return totalCount;
