...
class EventEmitter {
    ...
    subscribe(eventName, callback) {
        if (this.memory.get(eventName)) {
            const listFuncsObj = this.memory.get(eventName);
            listFuncsObj.push({ myFunc: callback, isActive: true });
            this.memory.set(eventName, listFuncsObj);
        } else {
            this.memory.set(eventName, [{ myFunc: callback, isActive: true }]);
        }
        let idx = this.memory.get(eventName).length - 1;
        return {
            eventName,
            idx,
            unsubscribe: () => {
                const listFuncsObj = this.memory.get(eventName);
                listFuncsObj[idx].isActive = false;
                // this.memory.delete(eventName);
            },
        };
    }
    ...
    emit(eventName, args = []) {
        if (this.memory.get(eventName)) {
            const listFuncsObj = this.memory.get(eventName);
            const res = [];
            listFuncsObj.forEach((funcObj) => {
                if (funcObj.isActive) {
                    const funcAns = funcObj.myFunc(...args);
                    res.push(funcAns);
                }
            });
            return res;
